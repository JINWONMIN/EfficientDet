...
class Visualization():
    ...
    def __display(self, json_nm, json_data, img, obj_list, save_dir_path, imshow=True, imwrite=False):
        ...
        for annotation in annotations:
            ...
            print(json_nm, annotation['image_id'])
    ...
    def run(self, args):
        ...
        with open(args.jp, 'r', encoding='utf-8') as f:
            tmp = json.load(f)
            f.close()
        ...
        for img_path in img_list:
            ...
            img_nm = os.path.basename(img_path)
            img_id = [a['id'] for a in tmp['images'] if os.path.splitext(img_nm)[0] in a['file_name']]
            for json_path in json_list:
                json_nm = json_path['image_id']
                if img_id == json_nm:
                    json_str = ""
...
def add_arg():
    ...
    parser.add_argument("--jp", required=False, type=str, help="label_json path", default="../data/PnID/V01_DSME_Project 2022-08-03 14_54_01/annotations/instances_test.json")
    parser.add_argument("--sp", required=False, type=str,
                        help="save path", default="./V01_test_220901")
