...
from utils.utils import STANDARD_COLORS, standard_to_bgr
...
class Visualization():
    ...
    def __load_conf(self):
        with open('config/202210_01/V04/V04_str.json', 'r') as f:
    ...
    # Draw bbox on image
    def __display(self, json_nm, json_data, img, obj_list, save_dir_path, type_, imshow=True, imwrite=False):
        ...
        os.makedirs(os.path.join(save_dir_path, type_), exist_ok=True)
        for annotation in annotations:
            ...
            if len(target_list) == 0:
                ...
                if imwrite:
                    cv2.imwrite(os.path.join(save_dir_path, type_, file_nm), img)
                    print("image id:", annotation['image_id'], "\t", "annotation id:", annotation['id'])
    ...
    def run(self, args):
        ...
        img_list = [img for img in glob.glob(f'{args.ip}/{args.type}/*.png', recursive=True)]
        ...
        for img_path in img_list:
            ...
            for json_path in set([a['image_id'] for a in json_list]):
                ...
                if img_id == json_nm:
                    ...
                    self.__display(json_nm, json_str, img, obj_list, save_dir_path, args.type, imshow=False, imwrite=True)


def add_arg():
    parser = argparse.ArgumentParser(description="visualize labeling data")
    parser.add_argument("--ip", required=False, type=str, help="image path", default="./data/PnID_updated_202210_01/V02/ori_split")
    parser.add_argument("--jp", required=False, type=str, help="label_json path", default="./data/PnID_updated_202210_01/V02/annotations/ori_instances_train.json")
    parser.add_argument("--sp", required=False, type=str, help="save path", default="./data/PnID_updated_202210_01/visualization/origin/V02")
    parser.add_argument("--type", required=False, type=str, default="train")
